...
class CalendarApp:
    ...
    def upper_rows_setup(self):
        ...
        # self.center_lbl = Label(self.top_frm, text=f'{Months(hlp.get_current_month()).name} {hlp.get_current_year()}') \
        self.center_lbl = Label(self.top_frm, text=f'{self.month.current_month} {self.month.current_year}') \
            .grid(row=0, column=1)
    ...
    def main_view_setup(self):
        ...
        row = 0
        column = 0
        for cnt, day in enumerate(self.month.get_day_by_day(), start=1):
            if column == 0:
                Label(self.left_frm, width=3, height=4, text=f'v\n{day[1]}', anchor=N, relief=GROOVE) \
                    .grid(row=row, column=column)

            column = int(day[2])
            if day[2] == 7 or day[4]:
                Label(self.main_frm, width=12, height=4, text=f'{hlp.get_date_from_string(day[0])}\n{day[3]}',
                      anchor=N, relief=GROOVE, bg='#E61010').grid(row=row, column=column)
            else:
                Label(self.main_frm, width=12, height=4, text=f'{hlp.get_date_from_string(day[0])}\n{day[3]}',
                      anchor=N, relief=GROOVE).grid(row=row, column=column)
