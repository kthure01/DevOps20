...
you'd need to choose them like this:
    ...
    it directly would need to have a joltage rating of 1, 2, or 3 jolts. Of these, only one you have is an
...
def part1():
    ...
    lower = 0
    for cnt, item1 in enumerate(jolt, start=0):
        if item1 == lower + 1:
            lower = item1
            jolt_result[1].append(item1)
        elif item1 == lower + 3:
            lower = item1
            jolt_result[3].append(item1)

        if cnt + 1 == len(jolt):
            jolt_result[3].append(item1 + 3)

    print(len(jolt_result[1]), len(jolt_result[3]), len(jolt_result[1]) * (len(jolt_result[3])))
...
def part2():
    ...
    for cnt, item1 in enumerate(jolt, start=0):
        ...
        if item1 == lower + 1:
            lower_lst.append(item1)
            jolt_result[1].append(item1)
        elif item1 == lower + 3:
            lower_lst.append(item1)
            jolt_result[3].append(item1)

        if lower_lst.__len__() > 0:
            lower = lower_lst[0]
            lower_lst.clear()
